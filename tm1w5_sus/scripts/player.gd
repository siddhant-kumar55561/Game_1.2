...
func _physics_process(delta: float) -> void:
	# 0. Make sure animation tree is active
	...
	# 6. Attack handled as last action "in frame".
	_handle_attacks()
...
func _handle_attacks():
	...
	if combo_queued:
		if is_on_floor():
			if current_state != "lightattackplus":
...
func _update_animation():
	...
	if not is_on_floor():
		if velocity.y < 0:
			if current_state != "jumpUp":
				state_machine.travel("jumpUp")
		else:
			if current_state != "jumpDown":
